...
case "$CMD" in
  ...
  help|*)
    ...
    echo ""
    echo "Graph (EDN 트리플) — native binary 또는 clj:"
    ...
    echo ""
    echo "Stem (JVM 전용 — Kiwi JNI, native-image 불가):"
    echo "  stem <\"문장\"> [--tokens]      단건 어간 추출 + expand"
    echo "  stem --batch                   stdin 배치 (andenken 인덱싱용)"
    echo "  stem --serve [port]            소켓 서버 (쿼리당 ~1ms)"
    echo "  stem inject                    사전 주입 테스트"
    echo ""
    echo "Build:"
    echo "  build [--output PATH] [--force]  GraalVM native binary (stem 제외)"
    echo "  native-build                     GraalVM 직접 (nix develop 안에서)"
    echo "  jar-build                        JVM uberjar (stem 제외)"
    echo ""
    echo "관계 타입:"
